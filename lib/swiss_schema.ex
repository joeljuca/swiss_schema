defmodule SwissSchema do
  ...
  @doc """
  Inserts all entries into the data store.

  It expects a list of entries to be inserted as argument, either as keyword
  lists or as maps. The keys of the entries are the field names as atoms and
  the value should be the respective value for the field type or, optionally,
  ...
  as second element. If the database does not support `RETURNING` in `INSERT`
  statements or no return result was selected, the second element will be
  `nil`.

  If the schema primary key has type `:id` or `:binary_id`, it will be handled
  either at the adapter or the storage layer. However any other primary key
  type or autogenerated value, like `Ecto.UUID` and timestamps, won't be
  autogenerated when using `c:insert_all/2`. You must set those fields
  explicitly. This is by design as this function aims to be a more direct way
  to insert data into the database without the conveniences of `c:insert/2`.
  This is also consistent with `c:update_all/2` that does not handle
  autogenerated values as well.

  It is also not possible to use `c:insert_all/2` to insert across multiple
  tables, therefore associations are not supported.

  ## Options

    * `:returning` - selects which fields to return. When `true`, returns all
      fields in the given schema. May be a list of fields, where a struct is
      still returned but only with the given fields. Or `false`, where nothing
      is returned (the default). This option is not supported by all databases.
    ...
    * `:prefix` - The prefix to run the query on (such as the schema path in
      ...
      query and any `@schema_prefix` set in the schema.

    * `:on_conflict` - It may be one of `:raise` (the default), `:nothing`,
      `:replace_all`, `{:replace_all_except, fields}`, `{:replace, fields}`,
      a keyword list of update instructions or an `Ecto.Query` query for
      updates. See the "Upserts" section for more information.

    * `:conflict_target` - A list of column names to verify for conflicts. It
      is expected those columns to have unique indexes on them that may
      conflict. If none is specified, the conflict target is left up to the
      database. It may also be `{:unsafe_fragment, binary_fragment}` to pass
      any expression to the database without any sanitization, this is useful
      for partial index or index with expressions, such as
  ...
  A query can be given instead of a list with entries. This query needs to
  select into a map containing only keys that are available as writeable
  columns in the schema.

  ## Examples

      Post.insert_all([[title: "My first post"], [title: "My second post"]])
  ...
  Upserts map to `"ON CONFLICT"` on databases like Postgres and
  `"ON DUPLICATE KEY"` on databases such as MySQL.
  ...
  MySQL will return, at a minimum, the number of entries attempted. Eg.: if
  `:on_conflict` is set to `:nothing`, MySQL will return the number of entries
  attempted to be inserted, even when no entry was added.

  Also note that if `:on_conflict` is a query, MySQL will return the number of
  attempted entries plus the number of entries modified by the `UPDATE` query.
  ...
  are using in the `:placeholders` option map. For example:
      ...
      Post.insert_all(entries, placeholders: placeholders)
